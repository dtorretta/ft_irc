...
void	Server::Channel_Exist(Channel *channel, Client *client, int fd, std::string key, std::string name)
{
	...
	if (count > 10)
	{
		_sendResponse(ERROR_IN_TOO_MANY_CHANNELS(client->GetNickName()), fd);
		return ;
	}
...
void	Server::JOIN(std::string cmd, int fd)
{
	...
	{
		_sendResponse(ERROR_TOO_MANY_TARGETS(client->GetNickName()), fd);
		return ;
	}
	else
	{
		for (size_t i = 0; i < token.size(); i++)
		{
			...
			{
				_sendResponse(ERROR_CHANNEL_NOT_EXISTS(client->GetNickName(), token[i].first), fd);
				return ;
			}
		}
	}
